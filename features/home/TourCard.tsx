          ...
          >
            ...
            <ul className="w-full text-left">
              {accActivity.entries.map((entry, innerIndex) => (
                <li
                  key={innerIndex}
                  className="m-1 flex align-middle w-full text-left items-center justify-start gap-2"
                >
                  <span
                  ...
                  >
                    {handleLinksInCard(entry.label, event)}
                  </span>
                </li>
              ))}
            </ul>
...
const handleLinksInCard = (label: string, event: Event) => {
  ...
  if (label.includes("<link:frokost>")) {
    const parts = label.split("<link:frokost>");
    return (
      <div className="flex items-center">
        {`${parts[0]} `}
  ...
  if (label.includes("<link:tour>")) {
    const parts = label.split("<link:tour>");
    return (
      <div className="flex items-center">
        {`${parts[0]} `}
  ...
  if (label.includes("<link:hotel>")) {
    const parts = label.split("<link:hotel>");
    return (
      <div className="flex items-center">
        {`${parts[0]} `}
  ...
  if (label.includes("<link:gf>")) {
    const parts = label.split("<link:gf>");
    return (
      <div className="flex items-center">
        {`${parts[0]} `}
        {handleLinks(yearCity, "gf")}
        {` ${parts.length > 1 && parts[1]}`}
      </div>
    );
  }
